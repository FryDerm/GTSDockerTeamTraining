/*
/webstore/checkout/index#pass-step-
This code handles the pass information section of single page checkout.
*/
...
gts.PassStep = function () {
	...
	var settings = {
		...
		confirmMemberDeleteMessage: "[L:ConfirmMemberDeleteMessage]"
	...
	*/
	...
	function setNewMemberDefaults(member, pass) {
        ...
        member.OrderLineId = getPassOrderLineId();
    ...
    function getProduct() {
        ...
        if (settings.cartItem.PackageDetails) {
            for (var p = 0; p < settings.cartItem.PackageDetails.length; p++) {
                var packageDetailItem = settings.cartItem.PackageDetails[p];
        ...
        if (!settings.passKindId) {
		    ...
		    if (passPackageDetail !== null) {
		        productData.plu = passPackageDetail.PLU;
		    }
    ...
    function renderAsCopyMember(highlightNewContact, member) {
    	var isRenewal = settings.cartItem.IsRenewal;
    	var removeLinkToAccount = settings.checkoutOptions.RemoveLinkToAccount;
        ...
        var memberViewModel = {
            ...
            ShowLinkToAccount: !(isRenewal && removeLinkToAccount)
        };
	...
	function load() {
	    ...
	    $.when(loadRelationshipTypesIntoCache(), gts.checkoutNameTitles.Get(), gts.checkoutNameSuffixes.Get(), loadMultipleChoicesIntoCache())
	        .done(function() {
	            gts.formGenerator.generate({
	                rules: primaryMemberFieldRules,
	                form: $(settings.selector).find(settings.passForm),
	                fieldInputPrefix: settings.primaryFieldInputPrefix,
	                renewalmode: settings.cartItem.IsRenewal,
                    disableAddress: settings.cartItem.IsDualJoint,
	                on: {
	                    addRule: on.addRule,
	                    addInputValues: onAddPassInputValues,
	                    afterInputAdded: onAfterInputAdded
	...
	function createPrimaryMemberObject(id) {
	    var newPrimaryMember = {
	        ...
	        OrderLineId: getPassOrderLineId(),
    ...
    function getPassOrderLineId() {
        ...
        return passPackageDetail !== null ? passPackageDetail.Id : settings.cartItem.Id;
    }
	...
	function onBrokenRulesError(brokenRules) {
	    if (!brokenRules || !brokenRules.length) {
	        if (!brokenRules.responseJSON) {
	            $("#ErrorMessage").text("An error occurred while processing your request.");
	        } else {
	            $("#ErrorMessage").text(brokenRules.responseJSON.Message);
	        }
	        return;
		}
	...
	function validateJointMembershipInfo() {
	    ...
	    } else if (settings.passKind.MaxChildren !== -1 && children.length > settings.passKind.MaxChildren) {
	    ...
	    } else if (settings.passKind.MaxAddons !== -1 && addons.length > settings.passKind.MaxAddons) {
	...
	function editMember(member, inputPrefix, dialogSelector, rules) {
		...
		gts.formGenerator.generate({
			...
			on: {
				addRule: on.addRule,
				addInputValues: onAddPassInputValues,
				afterInputAdded: function (rule, input) { onAfterInputAdded(rule, input, null, member.OverrideReadOnlyName); }
			}
		}).done(function() {
		    populateCountries(inputPrefix)
                .done(function () {
	...
	function removeAdult(e) {
	    ...
	    if (!confirm(settings.confirmMemberDeleteMessage)) {
	        return;
	    }
	...
	function canAddMoreAddons() {
		if (settings.passKind.MaxAddons > 0)
			return addons.length < settings.passKind.MaxAddons;
		else if (settings.passKind.MaxAddons < 0)
			return true;
	...
	function populateAdultForm() {
		populateForm(settings.adultFieldInputPrefix, settings.addAdultDialog, adultMemberFieldRules, true);
	}

	function populateChildForm() {
		populateForm(settings.childFieldInputPrefix, settings.addChildDialog, childMemberFieldRules, true);
	}

	function populateForm(inputPrefix, dialogSelector, rules, enableNames) {
		...
		gts.formGenerator.generate({
			...
			on: {
				addRule: on.addRule,
				addInputValues: onAddPassInputValues,
				afterInputAdded: function (rule, input) { onAfterInputAdded(rule, input, null, enableNames); }
	}
		}).done(function() {
    ...
    /* load MultipleChoices into Cache */
	function loadMultipleChoicesIntoCache() {
	    return gts.eGalaxyWebAPI.multipleChoices.get(function (data) {
	        multipleChoicesCache = data;
	    });
	}

    /* Populate MultipleChoices options to user field input */
	function populateMultipleChoiceInput(input, rule, multipleChoices) {
	    input.empty();
	    ...
	    for (var i = 0; i < multipleChoices.length; i++) {
	        ...
	        if (multipleChoice.MultipleChoiceID === rule.MChoiceID) {
	            var option = $("<option></option>").text(multipleChoice.ResponseText).val($.trim(multipleChoice.ResponseText));
    ...
    /* Populate MultipleChoices to user fields */
	function populateMultipleChoices(input, rule, selectedValue) {

	    if (rule.MChoiceID === 0) {
	        return $.when();
	    }

	    if (multipleChoicesCache) {
	        populateMultipleChoiceInput(input, rule, multipleChoicesCache);
	        if (selectedValue) {
	            input.val(selectedValue);
	        }
	        return $.when();
	    } else {
	        return loadMultipleChoicesIntoCache()
                .done(function () {
                    populateMultipleChoiceInput(input, rule, multipleChoicesCache);
                    if (selectedValue) {
                        input.val(selectedValue);
	...
	function setNewMemberFields(fieldPrefix, memberDemographicType, rules) {
	    ...
	    member.Fields = {};
		member.OverrideReadOnlyName = true;		//  You can always edit new member names regardless of membership renewals or not
	...
	function onAfterInputAdded(rule, input, disableAddress, enableNames) {
	    // NOTE:add here
	    if (rule.MChoiceID) {
	        populateMultipleChoices(input, rule);
	    }
		if (rule.Required) {
			input.closest(".form-field-input").addClass("required");
		}
        // disable address for joint members (Dual membership)
		if (disableAddress) {
		    if (rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.Street1 ||
		            rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.Street2 ||
		            rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.PostalCode ||
		            rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.City
		    ) {
		        input.attr('readonly', true);
		        input.addClass('input-disabled');
		    }
		    else if (rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.State) {
		        input[0].childNodes[0].setAttribute('disabled', true);
		        input[0].childNodes[0].setAttribute('class', "input-disabled"); // for state dropdown
		        input[0].childNodes[1].setAttribute('readonly', true); // for state textbox
		        input[0].childNodes[0].setAttribute('class', "input-disabled");
		        input.addClass('input-disabled');
            }
		    else if (rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.CountryCode) {
		...
		if (rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.FirstName ||
		    rule.FieldNumber === gts.eGalaxyWebAPI.Pass.Fields.LastName) {
			if (settings.cartItem.IsRenewal && !enableNames) {
				input.attr('readonly', true);
		        input.addClass('input-disabled');
		    }
		}
